-- Trim the description in permits_near_map's output to cut Supabase egress.
--
-- permits_near_map returns up to ~1,000 marker rows per uncached request and is
-- the largest single contributor to bandwidth. Descriptions run to whole
-- paragraphs; a marker popup only needs the gist. The sidebar's permits_near()
-- keeps full text (its rows are the ones a user actually reads), so a permit
-- that is both listed and mapped still has its full description available.
--
-- Body-only change, signature unchanged, so create-or-replace is fine.

create or replace function permits_near_map(
